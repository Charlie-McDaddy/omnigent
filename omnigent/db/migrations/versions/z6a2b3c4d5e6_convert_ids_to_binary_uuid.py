...
def _upgrade_postgresql() -> None:
    ...
    for table, cols in _BINARY_ID_COLUMNS.items():
        for col in cols:
            op.execute(
                sa.text(
                    f'ALTER TABLE "{table}" ALTER COLUMN "{col}" TYPE bytea '
                    f"USING decode(right(replace(\"{col}\", '-', ''), 32), 'hex')"
                )
            )


def _upgrade_mysql(nullable: dict[tuple[str, str], bool]) -> None:
    """Reinterpret as binary, UNHEX the trailing 32 chars, then fix to BINARY(16).

    ``UNHEX()`` returns NULL — not an error — on malformed input, so each
    column's NULL count is compared before/after the UPDATE and any increase
    aborts the migration. This matches the fail-loud behaviour of the SQLite
    (``bytes.fromhex``) and PostgreSQL (``decode``) paths instead of silently
    nulling a bad id.
    """
    bind = op.get_bind()
    for table, cols in _BINARY_ID_COLUMNS.items():
        for col in cols:
            ...
            op.execute(sa.text(f"ALTER TABLE `{table}` MODIFY `{col}` VARBINARY(64) NULL"))
            op.execute(
                sa.text(
                    f"UPDATE `{table}` SET `{col}` = UNHEX(RIGHT(REPLACE(`{col}`, '-', ''), 32)) "
            ...
            if nulls_after != nulls_before:
                raise RuntimeError(
                    ...
                    f"value(s) in `{table}`.`{col}` are not valid 32-char hex uuids "
                    f"(UNHEX returned NULL); aborting before the type change"
                )
