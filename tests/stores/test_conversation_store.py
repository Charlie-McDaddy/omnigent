...
def test_subagent_conversations_are_isolated(
    ...
    assert len(page_a.data) == 2, (
        f"Expected 2 items in 94c349190e241f85a984b3df8f129696, got {len(page_a.data)}. "
        "If > 2, items from bfcc6c068875253adf2f20bf30a19015 leaked into 94c349190e241f85a984b3df8f129696's listing."
    )
    texts_a = [item.data.content[0]["text"] for item in page_a.data]
    assert texts_a == ["alpha input", "alpha output"], (
        f"Expected alpha items only in 94c349190e241f85a984b3df8f129696, got {texts_a}. "
    ...
    for item in page_a.data:
        assert item.response_id == "task_a", (
            f"Item {item.id} in 94c349190e241f85a984b3df8f129696 has response_id {item.response_id!r}, expected 'task_a'."
        )
    ...
    assert len(page_b.data) == 2, (
        f"Expected 2 items in bfcc6c068875253adf2f20bf30a19015, got {len(page_b.data)}. "
        "If > 2, items from 94c349190e241f85a984b3df8f129696 leaked into bfcc6c068875253adf2f20bf30a19015's listing."
    )
    texts_b = [item.data.content[0]["text"] for item in page_b.data]
    assert texts_b == ["bravo input", "bravo output"], (
        f"Expected bravo items only in bfcc6c068875253adf2f20bf30a19015, got {texts_b}. "
        "If alpha items appear, the conversation_id filter is broken."
    )
    for item in page_b.data:
        assert item.response_id == "task_b", (
            f"Item {item.id} in bfcc6c068875253adf2f20bf30a19015 has response_id {item.response_id!r}, expected 'task_b'."
        )
