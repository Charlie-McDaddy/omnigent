...
async def test_session_snapshot_includes_model_options_from_runner(
    ...
    conv = Conversation(
        id="3e96c743939d90bc3ba238609904ef07",
        created_at=1,
        updated_at=1,
        root_conversation_id="3e96c743939d90bc3ba238609904ef07",
        agent_id="087b7cb7ac30abf4debfaa578d052ec6",
        labels={
    ...
    conv_store = _ConversationStore(
        [_message_item("item_1", "hi")],
        conversations={"3e96c743939d90bc3ba238609904ef07": conv},
    )

    first = await _get_session_snapshot(
        conv_store,  # type: ignore[arg-type]
        "3e96c743939d90bc3ba238609904ef07",
    )
    assert first.model_options == []
    await _drain_model_options("3e96c743939d90bc3ba238609904ef07")
    snapshot = await _get_session_snapshot(
        conv_store,  # type: ignore[arg-type]
        "3e96c743939d90bc3ba238609904ef07",
    )
    ...
    assert (
        "/v1/sessions/3e96c743939d90bc3ba238609904ef07/codex-model-options"
